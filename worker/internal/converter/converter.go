...
func (p *Pipeline) Convert(ctx context.Context, opts Options) ([]Segment, error) {
	c, ok := p.converters[opts.Format]
	if !ok {
		return nil, fmt.Errorf("unsupported format %q", opts.Format)
	}
	return c.Convert(ctx, opts)
}

// writeSegment escribe un segmento markdown en conceptos/ con su encabezado.
